...
1. Navigate to `Book Stack/Hamburger Menu` -> `Home` -> `Administration` -> `Entities Configuration` -> `Licensing`
...
1. Navigate `Book Stack/Hamburger Menu` -> `Home` -> `Administration` -> `Alert Rules` and Select “Security Analytics” from the Drop-Down Menu
...
1. Fill in the form as shown below and click “Create” to add your first “Email Channel”

	![](images/300/300-011c.png)
