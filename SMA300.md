...
- On the Correlation Rules screen, take a moment to explore some of the built-in correlation rules available within SMA for the purposes of threat detection. In today’s lab we will exercise three threats under the Host category
 ...
 1. Host `<-- Targeted Account Attack`
 1. Host `<-- Multiple Failed Logins`
 1. Host `<-- Brute Force Attack`
...
The data uploaded provides insigts into key steps that make up what is known as a `Kill Chain`: `Anomaly Detection` --> `Reconaissance` --> `Infiltration` --> `Lateral Movement`.
